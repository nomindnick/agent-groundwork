...
**Phase 4 — CLI frontend, complete.** All Phase 1/2/3 foundations plus `agent_groundwork/cli.py`, `agent_groundwork/__main__.py`, and a rewritten realistic `sandbox/AGENT.md`. `python -m agent_groundwork` starts an interactive REPL that streams the agent's event stream to a terminal with ANSI colors, wires `ask_user` to a stdin-backed input provider, handles Ctrl-C cancellation cleanly (cancels in-flight, returns to prompt), supports `/quit`/`/exit`/`/help`/`/trace` slash commands, and prints the trace file path on exit. The CLI never reaches into agent-core internals — communication is exclusively through the event stream and the injected input callback, which is what makes Phases 6/7 (web/Telegram frontends) a swap rather than a rewrite. Default model remains `gemma4:e4b`; the **Phase 2 decision** is recorded in `bakeoff_results/20260407-221909/decision.md` — finalists are `gemma4:e4b` (primary default), `gemma4:26b` (bigger same-family), and `gemma4:e2b` (fast specialist / candidate compaction model). Next is **Phase 5** (use-and-tune the system prompt and compaction settings against the three finalists in real multi-step tasks, updating `decision.md` with what's learned).
